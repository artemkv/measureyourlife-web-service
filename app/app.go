...
func SetupRouter(router *gin.Engine, allowedOrigin string) {
	...
	// do business
	router.GET("/daystats/:dt", reststats.HandleEndpointWithStats(
		withAuthentication(handleGetDayStats)))
	router.POST("/daystats/:dt", reststats.HandleEndpointWithStats(
		withAuthentication(handlePostDayStats)))
